...
class ZigToolchain:
    ...
    def compile(self,
        ...
        console.print('[ Compiling ]', ' '.join(compile_cmd))
        threading.Thread(target=self.__rc, args=(compile_cmd,)).start()
    ...
    def link(self,
        ...
        console.print('[ Linking ]', ' '.join(link_cmd))
        self.__rc(link_cmd)
    ...
    def archive(self,
        ...
        if not input_files:
            print("Fatal Error: No input files")
            raise SystemExit
        archive_cmd = (['zig', 'build-lib'] + input_files + ['-femit-bin=' + output])
        console.print('[ Archiving ]', ' '.join(archive_cmd))
